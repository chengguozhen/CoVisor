...
class ParallelQueryTopo(Topo):

    def __init__(self, enable_all = True, sw_number = 2, host_per_sw = 5,  topoFile = ""):
        '''
        Create topology with multiple switches for mininet.  IPs assigned
        to work with exprParallel() in queries.py.
        '''

        # Add default members to class.
        super(ParallelQueryTopo, self).__init__()
        ...
        for switch in self.graph.nodes():
            # Hardcode IP addresses to test forwarding and stats queries.
            #host = 'h_%s_1' % switch
            #ip = '2.0.0.1'
            #mac = self.graph.node[switch]['dpid'][4:-1] + str(2)
            #h = self.addHost(host, ip=ip, mac=mac)
            #self.addLink(h, switch)

            #host = 'h_%s_2' % switch
            #ip = '2.0.0.2'
            #mac = self.graph.node[switch]['dpid'][4:-1] + str(3)
            #h = self.addHost(host, ip=ip, mac=mac)
            #self.addLink(h, switch)

            #for i in range(2, host_per_sw):
