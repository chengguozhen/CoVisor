...
public class VirtualTopology {
    ...
    private PlumbingGraph buildTopology() {
    	PlumbingGraph graph = new PlumbingGraph(null);
    	graph.addNode(1);
    	graph.addNode(2);
    	graph.addNode(3);
    	graph.addPort(1, (short) 1);
    	graph.addPort(1, (short) 2);
    	graph.addPort(1, null);
    	graph.addPort(2, null);
    	graph.addPort(2, null);
    	graph.addPort(3, null);
    	graph.addPort(3, (short) 3);
    	graph.addPort(3, (short) 4);
    	graph.addEdge(1, (short) 3, 2, (short) 1);
    	graph.addEdge(2, (short) 2, 3, (short) 1);
	return graph;
    }
    ...
    public void exprHelperIP(int ipCount, int macExternal, int macInternal, int ipUpdate, List<String> macs, List<String> ips,
    	...
    	// build topology
	PlumbingGraph graph = buildTopology();
    	
    	// ip router
    	for (OFFlowMod fm : ipRouterRules) {
    		//System.out.println(fm);
    		graph.update(fm, graph.getNode(1));
    	}
    	
    	// gateway switch
    	for (OFFlowMod fm : gatewayRules){
    		//System.out.println(fm);
	    graph.update(fm, graph.getNode(2));
    	}
    	
    	// mac learner
    	for (OFFlowMod fm : macLearnerRules){
    		//System.out.println(fm);
    		graph.update(fm, graph.getNode(3));
    	}
    	...
    	for (int i = 0; i < ipUpdate; i++) {
			...
			PolicyUpdateTable updateTable = graph.update(fm, graph.getNode(1));
    	...
    	for (int i = 0; i < ipUpdate; i++) {
			OFFlowMod fm = ipRouterRules.get(i);
			((OFActionOutput) fm.getActions().get(0)).setPort((short) 2);
    ...
    public void exprHelperMAC(int ipCount, int macExternal, int macInternal, int macUpdate, List<String> macs, List<String> ips,
    	...
    	if (aclOpt) {
    		{
    			...
    			graph.addNode(1, storeTypes, storeKeys);
    		}
    		{
    			List<PolicyFlowModStoreType> storeTypes = new ArrayList<PolicyFlowModStoreType>();
    			List<PolicyFlowModStoreKey> storeKeys = new ArrayList<PolicyFlowModStoreKey>();
    			...
    			storeKeys.add(PolicyFlowModStoreKey.NW_DST);
    			storeKeys.add(PolicyFlowModStoreKey.ALL);
    			graph.addNode(2, storeTypes, storeKeys);
    		}
    		{
    			List<PolicyFlowModStoreType> storeTypes = new ArrayList<PolicyFlowModStoreType>();
    			List<PolicyFlowModStoreKey> storeKeys = new ArrayList<PolicyFlowModStoreKey>();
    			...
    			storeTypes.add(PolicyFlowModStoreType.WILDCARD);
    			storeKeys.add(PolicyFlowModStoreKey.DL_DST);
    			storeKeys.add(PolicyFlowModStoreKey.ALL);
    			graph.addNode(3, storeTypes, storeKeys);
    		}
    	} else {
    		graph.addNode(1);
    		graph.addNode(2);
    	...
    	for (int i = 0; i < macUpdate; i++) {
    		...
    		PolicyUpdateTable updateTable = graph.update(fm, graph.getNode(2));
    		fmCount += updateTable.addFlowMods.size();
			fmCount += updateTable.deleteFlowMods.size();
    	}
    	for (OFFlowMod fm : macLearnerUpdateRules){
			PolicyUpdateTable updateTable = graph.update(fm, graph.getNode(3));
			fmCount += updateTable.addFlowMods.size();
			fmCount += updateTable.deleteFlowMods.size();
    	}
    	long elapseTime = System.nanoTime() - startTime;
    ...
    private List<OFFlowMod> initIPRouterRules(int count) {
    	...
    	for (int i = 0; i < 254 && i < count; i++) {
    		String rule = String.format("priority=16,ether-type=2048,dst-ip=%d.0.0.0/16,actions=output:1", i + 2);
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
    	for (int i = 254; i < 254*256 && i < count; i++) {
    		String rule = String.format("priority=16,ether-type=2048,dst-ip=%d.%d.0.0/16,actions=output:1", i % 254 + 2, i / 254);
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
    	
    	String rule = "priority=16,ether-type=2048,dst-ip=1.0.0.0/16,actions=output:3";
    	flowMods.add(OFFlowModHelper.genFlowMod(rule));
    ...
    private List<OFFlowMod> initGatewayRules(int count, List<String> ips, List<String> macs) {
    	...
    	for (int i = 0; i < count; i++) {
    		String rule = String.format("priority=1,ether-type=2048,inport=1,dst-ip=%s,"
					+ "actions=output:2,actions=set-src-mac:11:11:11:11:11:11,actions=set-dst-mac:%s",
					ips.get(i), macs.get(i));
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
    	
    	String rule = "priority=1,ether-type=2048,inport=2,actions=output:1";
    	flowMods.add(OFFlowModHelper.genFlowMod(rule));
    ...
    private List<OFFlowMod> initMACLearnerRules(int external, int internal,  List<String> macs) {
    	...
    	for (int i = 0; i < external; i++) {
    		String rule = String.format("priority=1,inport=1,"
    				+ "src-mac=11:11:11:11:11:11,dst-mac=%s,actions=output:3", macs.get(i));
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
    	for (int i = 0; i < external; i++) {
    		String rule = String.format("priority=1,inport=3,"
    				+ "src-mac=%s,dst-mac=11:11:11:11:11:11,actions=output:1", macs.get(i));
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
    	for (int i = 0; i < internal; i++) {
    		String rule = String.format("priority=1,inport=2,"
    				+ "src-mac=00:00:00:00:11:11,dst-mac=%s,actions=output:3", macs.get(i));
    		flowMods.add(OFFlowModHelper.genFlowMod(rule));
    	}
